...
#define SENSOR_TYPE_AC          0
#define SENSOR_TYPE_NORMAL      1

static int sensor_type = SENSOR_TYPE_AC;
...
static int32_t ov5647_sensor_model_match(struct msm_sensor_ctrl_t *s_ctrl)
{

	if(sensor_type == SENSOR_TYPE_NORMAL)
	{
		strncpy((char *)s_ctrl->sensor_name, "23060113FA-OV-S", strlen("23060113FA-OV-S"));
	}
	else
	{
		strncpy((char *)s_ctrl->sensor_name, "23060110FA-OV-S", strlen("23060110FA-OV-S"));
	}
		...
		const struct i2c_device_id *id)
{
	...
	/* default value is SENSOR_TYPE_AC */
	sensor_type = SENSOR_TYPE_AC;
	
	rc = msm_sensor_i2c_probe(client, id);
	if(rc < 0)
	{	
		pr_err("%s: original AC ov5647 probe failed, to probe another normal ov5647\n", __func__);
