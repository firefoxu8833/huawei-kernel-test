...
static void __init virtualkeys_init(void)
{
    ...
    {
    	buf_vkey_size = sprintf(buf_virtualkey,
        			__stringify(EV_KEY) ":" __stringify(KEY_BACK)  ":80:900:160:80"
        		   ":" __stringify(EV_KEY) ":" __stringify(KEY_HOME)   ":270:900:160:80"
        		   ":" __stringify(EV_KEY) ":" __stringify(KEY_MENU) ":460:900:160:80"
        		   "\n"); 
    }
