...
/* merge the patch to fix freeze when reboot */
...
/* merge the patch to fix crash when reboot */
...
static int __init rpcrouter_init(void)
{
	...
	/* merge the patch to fix freeze when reboot */
	ret = register_reboot_notifier(&msm_rpc_reboot_notifier); //added by hujun 20121007
	if (ret)
